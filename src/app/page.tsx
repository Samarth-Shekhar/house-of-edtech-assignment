    ...
    <div className="min-h-screen bg-[var(--color-bg-primary)] text-[var(--color-text-primary)]">
      <nav className="sticky top-5 z-50">
        <div className="page-container">
          <div className="mx-auto flex h-16 max-w-[1400px] items-center justify-between gap-4 rounded-full border border-[var(--color-border-subtle)] bg-white/95 px-5 shadow-[0_18px_48px_rgba(17,24,39,0.08)] backdrop-blur-xl">
            <Link href="/" className="flex items-center gap-3 no-underline">
              <span className="flex h-11 w-11 items-center justify-center rounded-full bg-[var(--color-accent-mustard)] text-[#1f1f1f] shadow-sm">
                <Brain size={18} />
              </span>
      ...
      <main>
        <section className="page-container grid min-h-[calc(100vh-120px)] items-start gap-14 py-16 lg:grid-cols-[0.95fr_1.05fr] lg:py-20">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-6 text-5xl font-semibold leading-[1.02] text-[var(--color-text-primary)] sm:text-6xl lg:text-7xl">
              Hire with more clarity, less noise.
            </h1>

            <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-[var(--color-border-subtle)] bg-white px-4 py-2 text-sm font-semibold text-[var(--color-text-secondary)] shadow-sm">
              <span className="h-2.5 w-2.5 rounded-full bg-[var(--color-accent-mustard)]" />
              One calm workspace for screening, notes, and interviews
            </div>

            <p className="mt-6 max-w-xl text-base leading-8 text-[var(--color-text-secondary)]">
            ...
            <div className="mt-10 grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div
                  key={item.label}
                  className="rounded-[24px] border border-[var(--color-border-subtle)] bg-white px-5 py-5 shadow-[0_14px_36px_rgba(17,24,39,0.08)]"
                >
          ...
          <div className="rounded-[32px] border border-[var(--color-border-subtle)] bg-white shadow-[0_24px_80px_rgba(17,24,39,0.08)] p-6 sm:p-8">
            <div className="mb-5 flex items-center justify-between gap-4">
              ...
              <div className="rounded-full border border-[var(--color-border-subtle)] bg-[var(--color-bg-primary)] px-3 py-1.5 text-[11px] font-semibold text-[var(--color-text-secondary)]">
                Live view
              </div>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
                ...
                <div className="soft-surface p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-3xl font-semibold text-[var(--color-text-primary)]">4 active</div>
                    </div>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-3">
                    {[
                      ...
                      <div
                        key={item.label}
                        className="rounded-[18px] border border-[var(--color-border-subtle)] bg-white px-4 py-4"
                      >
                        <div
                          className={`mb-3 h-2 rounded-full ${
                            item.tone === "dark"
                              ? "bg-[#1f1f1f]"
                              : item.tone === "gold"
                              ? "bg-[var(--color-accent-mustard)]"
                              : "bg-[var(--color-bg-tertiary)]"
                          }`}
                        />
                        <div className="text-xl font-semibold text-[var(--color-text-primary)]">{item.value}</div>
              ...
              <div className="dashboard-card p-5 bg-[var(--color-bg-surface)]">
                ...
                <div className="space-y-3">
                  {tasks.map((task) => (
                    <div
                      key={task.title}
                      className="flex items-center gap-3 rounded-[20px] border border-[var(--color-border-subtle)] bg-white px-3 py-3"
                    >
                      <span
                        className={`flex h-8 w-8 items-center justify-center rounded-full ${
                          task.done
                            ? "bg-[rgba(212,165,40,0.16)] text-[var(--color-accent-mustard)]"
                ...
                <div className="mt-4 rounded-[20px] border border-[var(--color-border-subtle)] bg-white p-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--color-text-muted)]">
                    Suggested next action
                  </p>
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <div className="text-sm font-semibold text-[var(--color-text-primary)]">
                      Review 3 standout candidates
                    </div>
        ...
        <section className="page-container py-16">
          <div className="rounded-[32px] border border-[var(--color-border-subtle)] bg-white shadow-[0_24px_80px_rgba(17,24,39,0.08)] p-8">
            <div className="grid gap-6 lg:grid-cols-2">
              {[
                {
                  title: "PostgreSQL + Prisma",
                  description:
                    "Production-ready relational database support with role-based access, candidate data, and AI scores.",
                },
                {
                  title: "AI resume analysis",
                  description:
                    "Resume parsing, skill matching, bias scoring, and explainable candidate fit with Google Gemini.",
                },
                {
                  title: "Hiring pipeline",
                  description:
                    "Track candidates from application through screening, interview, offer, and hire in a unified dashboard.",
                },
                {
                  title: "Evaluation & audit trail",
                  description:
                    "Structured feedback, interviewer scoring, and secure audit logging for compliance and review.",
                },
              ].map((item) => (
                <div key={item.title} className="rounded-[24px] border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface)] p-6">
